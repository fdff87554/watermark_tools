...
func main() {
	...
	imgReader, err := os.Open("./test.jpeg")
	if err != nil {
		log.Fatal(err)
	}
	defer imgReader.Close()
	imgWriter, err := os.Create("./out_90.jpeg")
	if err != nil {
		log.Fatal(err)
	}
	defer imgWriter.Close()

	image, err := jpeg.Decode(imgReader)
	if err != nil {
		log.Fatal(err)
	}
	//if err := jpeg.Encode(imgWriter, image, &jpeg.Options{Quality: 100}); err != nil {
	//	log.Fatal(err)
	//}
	if err := jpeg.Encode(imgWriter, image, &jpeg.Options{Quality: 90}); err != nil {
		log.Fatal(err)
	}
}